...
| **`Sound Effects Mapping`** | • `VideoAnalyzer`: অ্যানালাইসিস শুরু (`scan`), রেজাল্ট লোড (`success`), থাম্বনেইল ডাউনলোড (`download`), ট্যাগ/কিওয়ার্ড/হ্যাশট্যাগ কপি (`copy`), এরর (`error`), স্যাম্পল ক্লিক (`click`)<br>• `ChannelAnalyzer`: অ্যানালাইসিস শুরু (`scan`), রেজাল্ট লোড (`success`), কপি (`copy`), এরর (`error`), স্যাম্পল ক্লিক (`click`)<br>• `AiOptimizerCard`: এআই রান (`generate`), অপটিমাইজেশন কমপ্লিট (`success`), কপি (`copy`), এরর (`error`)<br>• `TextTools`: টেক্সট কপি (`copy`), ৬ ফরম্যাটে ডাউনলোড (`download`), হিস্ট্রি ক্লিয়ার (`clear`), আনডু/ট্রান্সফর্ম (`click`)<br>• `FaviconGenerator`: সমস্ত রেজোলিউশন ও .ico ডাউনলোড (`download`)<br>• `PromptDetail`: প্রম্পট লাইক (`like`), প্রম্পট ও শেয়ার লিঙ্ক কপি (`copy`)<br>• `History`: হিস্ট্রি আইটেম ডিলিট (`delete`), অল হিস্ট্রি ক্লিয়ার (`clear`)<br>• **সাইলেন্ট ফিল্টার**: ক্যাটাগরি ও ট্যাব পরিবর্তনের অপ্রয়োজনীয় শব্দ রিমুভ করে স্মুথ ন্যাভিগেশন নিশ্চিত করা হয়েছে। |
