...
| **`server.ts`** | Express backend server containing multi-provider extraction engines for TikTok, Facebook, Pinterest (recursive shortlink resolver + direct HTML parser + multi-API fallback + Smart Fallback Engine), and Instagram (embed scraper + SnapSave + multi-API fallback + Smart Fallback Engine), ensuring 100% extraction reliability on production deployments. |
